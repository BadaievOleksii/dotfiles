...
if [ -x /usr/bin/tput ] && tput setaf 1 >&/dev/null; then
	...
	bg_red="\[$(/usr/bin/tput setab 1)\]"
	bg_black="\[$(/usr/bin/tput setab 0)\]"
	bg_white="\[$(/usr/bin/tput setab 7)\]"
	fg_bold="\[$(/usr/bin/tput bold)\]"
...
function prompt_command {
	...
	if $color_is_on; then
		...
		if [[ ! -z $GIT_BRANCH ]]; then
			if [[ -z $GIT_DIRTY ]]; then
				PS1_GIT=" (${color_red}*${color_off}${color_blue}${GIT_BRANCH::15}${color_off})"
			else
				PS1_GIT=" (${color_blue}${GIT_BRANCH::15}${color_off})"
			fi
		fi
